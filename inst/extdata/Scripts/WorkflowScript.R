...
#########################################
#### Add Array data ####
RawDataCompile(Fpath = file.path(homedir, "AppData"), outPath = file.path("./ProcessFiles/RawData.txt"), StartAt = 1, sleep = 60, DataType = "Array" )
...
#####################################################
RawSE <- GenerateRawSE(df = metaDF, ArrayDT=RawArrayComplete, GTFFpath = file.path(homedir, "OverviewFiles", "GTFHuman.gtf.gz"), overview=overview)
names(RawSE)
...
#### Create raw data summarizedExperiment object ####
#####################################################
RawSE <- GenerateRawSE(df = metaDF, ArrayDT=RawArrayComplete, GTFFpath = file.path(homedir, "OverviewFiles", "GTF.gtf.gz"), overview=overview)
names(RawSE)
saveRDS(RawSE[["RawSE"]], file=file.path(homedir, "ProcessFiles", "SumarizedExp_RawDB.rds"))
RawSE <- readRDS(file.path(homedir, "ProcessFiles", "SumarizedExp_RawDB.rds"))
assay(RawSE)[1:5,1:5]; head(rowData(RawSE)); head(colData(RawSE))
